...
下一步:核心 sim 闭环(P1–P11)+ CI 回归基线(P12,含 `baselineNsPerTick` 校准)+ 活跃列表换路(P13)+ 表现层 v1(P14)+ 传送带 lane 选边入料 + P15(玩家 WASD/相机跟随/手挖)+ debug 覆盖层 + `RotateEntity` + 机器输入过滤 + 无电机械臂抓取 bug + `ElectricGrid` 空间索引化(O(scale²) 根因修复)+ **Machines 休眠/唤醒** + **MiningDrills 休眠/唤醒** + **MCP 原子操作模拟驱动器(子项目一+子项目二)**已完成。P16(玩家碰撞盒 + 传送带带人移动)代码/审查已完成,在未合并分支上等人工 F5。剩余候选(无强依赖顺序,按价值/成本挑):① **`Inserters` 的休眠/唤醒**(两轮休眠/唤醒实践下来机制已经很成熟,但机械臂"背后是持续变化的传送带,没有干净的'有活可干'事件"这个难点还没碰过,是当初刻意留到最后的部分;鉴于 MiningDrills 那轮的实测性能收益低于预期且原因未查明,建议先花一轮 spike 搞清楚 `AwakeSnapshot`/`AsleepSnapshot` 分配开销是不是真正的瓶颈,再决定要不要接着做 Inserters);② **表现层后续**:视觉插值 lerp(要解决传送带物品跨 tick 身份匹配)、真美术(骨骼变换 vs 预渲染帧表)、完整命令 UI(快捷栏/背包/机器面板)、保留模式渲染(`TileMapLayer` + `MultiMesh`)、碰撞盒 debug 可视化(等 P16 合并)——各自单独 brainstorm。
