...
建造成本机制(2026-09-17,架构级,SDD 全流程 + subagent-driven-development 执行,5 个 task):放置建筑从免费变成消耗库存物品,拆除从瞬间生效变成长按+距离限制+返还物品——用户明确要求"这个游戏的核心循环就是自动化制造需要的东西"。设计上两个关键发现:①`ItemPrototype.PlaceResult` 字段早就存在但从没被读过,数据模型本来就预留了"物品→可放置实体"这层映射;②手挖机制(`MineStart`/`MineStop`)早就完整支持"长按+交互距离+返还物品",拆除不需要新机制,只要给建筑补 `minableResult` 并把右键接到这套已有逻辑上。核心实现:新命令 `BuildFromInventory`(距离+库存+占地三重检查,查 `PrototypeRegistry.TryGetEntityByName` 新方法解决"按名字查任意子类型实体"这个 `TryGet<T>` 做不到的事——`TryGet<T>` 按精确 CLR 类型做键,查不到抽象基类,这个坑是写 plan 时自查出来的);`PlaceEntity` 命令完全不变(继续免费,专供 bench/demo 场景搭建)。**执行期发现**(比预想大得多的意外):Task 2 加 7 个新物品/配方原型后,~20 个既有测试全挂——根因是这个项目的 `PrototypeRegistry` 按全局字母序给所有原型统一分配 id,新增任何原型都会导致后面所有 id 平移,连累 `ResourceGrid` 的矿脉生成(矿种选择用 `ResourcePrototype.Id`),矿脉分布全变了。裁定这是这套 id 分配设计(早于本次改动)的预期副作用,不是数据 bug,按项目一贯的"golden 重新基线"先例处理——独立验证过 `PrototypeRegistry.AssignIds()`/`ResourceGrid.cs` 源码坐实这个机制,而不是只信实现者的猜测;修复期间实现者自己触发过一次 API 限流中断,恢复后继续时又独立发现另外两类 fallout(建筑名字和物品/配方撞名是设计本身就要的、开局物资包新增煤影响了槽位数学),20 个失败全部排查完确认没有真 bug。另有 Task 4 review 抓出一个我自己写 plan 时留的测试坐标 bug(目标格超出交互距离,导致测的是距离拒绝不是库存拒绝,不是实现者的错)。**人工 F5 验收踩到这个会话第二次同类坑**:Task 5 静态审查全过,但第一次人工验收四项全部"失败"(不扣库存、瞬间拆除、无距离限制)——排查发现是 Godot 编辑器 F5 用的是 Debug 配置构建,而任务实现只重新编译了 Release,编辑器加载的是这整个专项开始之前的旧 DLL;重新编译 Debug 配置后二次验收全部通过。最终 5 个 task 全部 clean,整分支 final review(sonnet,opus 因会话限流失败后降级重跑)零 Critical/Important,3 个已知 Minor 全部推迟。commits `57a5324`..`b550571`。

下一步:核心 sim 闭环(P1–P11)+ CI 回归基线(P12,含 `baselineNsPerTick` 校准)+ 活跃列表换路(P13)+ 表现层 v1(P14)+ 传送带 lane 选边入料 + P15(玩家 WASD/相机跟随/手挖)+ debug 覆盖层 + `RotateEntity` + 机器输入过滤 + 无电机械臂抓取 bug + `ElectricGrid` 空间索引化(O(scale²) 根因修复,含 2026-09-14 这轮真正修完的建索引侧)+ **Machines 休眠/唤醒** + **MiningDrills 休眠/唤醒** + **MCP 原子操作模拟驱动器(子项目一+子项目二)**+ **P16(玩家碰撞盒 + 传送带带人移动,人工 F5 已补验)**+ **碰撞盒 debug 可视化**+ **建造成本机制**已完成。**`Inserters` 的休眠/唤醒经两轮 spike 验证后判定不值得做,已从候选里移除**;**60 UPS @ 2 万实体这个性能目标已达成**(见上条)。剩余候选:**表现层后续**(无强依赖顺序,各自单独 brainstorm):视觉插值 lerp(要解决传送带物品跨 tick 身份匹配)、真美术(骨骼变换 vs 预渲染帧表)、**完整命令 UI(快捷栏/背包/机器面板)**——建造成本机制已经把 `BuildFromInventory` 这个命令接口铺好,快捷栏子项目现在可以直接在它上面做"选建筑"的界面,不用再碰 sim 层、保留模式渲染(`TileMapLayer` + `MultiMesh`)、拆除/挖矿进度条 UI(用户已提出,待单独 brainstorm)。
