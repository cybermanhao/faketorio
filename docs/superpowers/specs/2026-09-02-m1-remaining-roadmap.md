...
下一步:核心 sim 闭环(P1–P11)+ CI 回归基线(P12,含 `baselineNsPerTick` 校准)+ 活跃列表换路(P13)+ 表现层 v1(P14)+ 传送带 lane 选边入料 + P15(玩家 WASD/相机跟随/手挖)+ debug 覆盖层 + `RotateEntity` + 机器输入过滤 + 无电机械臂抓取 bug + `ElectricGrid` 空间索引化(O(scale²) 根因修复)+ **Machines 休眠/唤醒**已完成。**"小项清理"三项 + P12 遗留步骤 + 前两项性能优化全部做完**。P16(玩家碰撞盒 + 传送带带人移动)代码/审查已完成,在未合并分支上等人工 F5。剩余候选(无强依赖顺序,按价值/成本挑):① **`MiningDrills`/`Inserters` 的休眠/唤醒**(把 Machines 这次跑通的机制(状态划分/安全网/事件驱动唤醒)推广过去——机械臂"背后是传送带"这种连续变化源、采矿机"矿脉挖空后要不要继续每 tick 探测"这类唤醒语义比 Machines 复杂,是当初刻意收窄范围留到后面的部分;单独 brainstorm,建议先跑一轮新的 UPS-vs-scale 曲线确认边际收益);② **表现层后续**:视觉插值 lerp(要解决传送带物品跨 tick 身份匹配)、真美术(骨骼变换 vs 预渲染帧表)、完整命令 UI(快捷栏/背包/机器面板)、保留模式渲染(`TileMapLayer` + `MultiMesh`)、碰撞盒 debug 可视化(等 P16 合并)——各自单独 brainstorm。
