...
`main` 已含 P12(CI 回归基线)+ P14(表现层 v1);P13(有序活跃 id 列表)在 PR #1 待合。`dotnet test sim/Faketorio.Sim.Tests` = **427 passing**;`dotnet test Faketorio.sln` 含 `Presentation.Core.Tests`;`dotnet build -c Release` = 0 警告 0 错误。SDD 执行时的逐任务账本在 `.superpowers/sdd/<plan>/progress.md`,收尾即删——**本表是唯一的跨 plan 进度看板**,plan 文档的 `- [ ]` 复选框不反映状态。
...
| 横切 · 传送带 lane 左右语义 + 按几何选边入料 | ⬜ 未立项 · **sim 改动,走 spec + golden gate**。现状:采矿机产出(`Simulation.cs:689`)/ 机械臂放件(`Simulation.cs:856`)入 lane 都是 `LaneA.TryInsert(...) \|\| LaneB.TryInsert(...)` 回退——A 满溢到 B,没有"固定喂某一侧"的规则,挂机久了两条 lane 都堆满。两条 lane 本身**不交互**(各自 `Advance`,交接同名对接 `Simulation.cs:100/118`),这部分已正确。要做的:给 `BeltLine.LaneA/LaneB` 定死相对 `Direction` 的**左/右**语义并与表现层 `WorldView.DrawLaneItems` 的左右偏移对齐;入料按源与带的几何关系确定性选边——源**同向**进料 → 放**运动方向右手侧**;源**正交**进料 → 放**远端 lane**(离源较远那条,即 Factorio 行为)。 | — |
...
下一步:核心 sim 闭环(P1–P11)+ CI 回归基线(P12)+ 表现层 v1(P14)已完成。剩余候选(无强依赖顺序,按价值/成本挑):① 实体休眠 / 活跃列表(§5.3 性能地基,P9/P10/P11 三个两趟扫描是共同落点;P12 的分阶段计时已量化优先级);② 机器输入库存加过滤 + "销毁掉落物品"统一处理 + 传送带 lane 左右语义 + 按几何选边入料(见横切表);③ `RotateEntity` 命令(仅在要让玩家事后转向时);④ `bench/golden.json` 的 `baselineNsPerTick` 校准(P12 遗留的一次性人工步骤,合并后从首次绿色 CI artifact 回填);⑤ **表现层后续**:玩家 WASD 控制 + 相机跟随移动的玩家、视觉插值 lerp(要解决传送带物品跨 tick 身份匹配)、真美术(骨骼变换 vs 预渲染帧表)、完整命令 UI(快捷栏/背包/机器面板)、保留模式渲染(`TileMapLayer` + `MultiMesh`)——各自单独 brainstorm。
