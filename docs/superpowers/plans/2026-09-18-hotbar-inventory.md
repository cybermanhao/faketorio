...
- **按键映射的一处偏离**：spec 原文写"`E` 键切换背包"，但 `project.godot` 里 `E` 键(keycode 69)已经绑定给 `player_mine`(手挖，`PlayerInputController.UpdateMining()`)——这是写 plan 时读代码才发现的真实冲突，spec 写的时候没有核对现有 `[input]` 映射。本 plan 改用 **`Tab` 键**切换背包面板(不跟任何现有映射冲突，是常见的"打开物品栏"按键)，直接在代码里判 `Keycode: Key.Tab`(同 `CameraController.cs` 判 `Key.M` 的写法，不新增 `project.godot` `[input]` 动作，不碰这个文件)。收尾任务的 F5 验收清单和完成后的 roadmap 记录都要写清楚这个键位偏离。
  **【Task 6 F5 验收期间用户改动,记于此供后续查阅】**：用户在 F5 验收前直接要求把这个偏离改回去——E 键手挖(`PlayerInputController.UpdateMining()`)整个下线(长按右键拆除/挖矿已经走同一套 `MineStart`/`MineStop` sim 命令,对矿脉和可拆实体都生效,功能完全等价,不是裁掉能力),`E` 键改回背包开关(`BuildController.cs` 里 `Key.Tab` 改成 `Key.E`),`project.godot` 的 `player_mine` action 一并删除(不再被任何代码引用)。这个改动**没有**走这份 plan 的 SDD 任务流程(不是这份 plan 原定范围的一部分),是 controller 直接在 Task 6 人工验收阶段之前完成的小改动，未经过 task-reviewer 审查——记录在这里避免以后读这份 plan 时对不上实际代码状态。
